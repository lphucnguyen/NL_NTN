        ...
        <div class="dashboard_graph">
            <div class="row x_title">
                ...
                <div class="col-md-6 text-right">
                    <div class="dropdown">
                        <button class="btn btn-transparent border py-1 dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        <div class="dropdown-menu p-3 mt-2 pull-right w-100" aria-labelledby="dropdownMenuButton">
                            <div class="row">
                                <!-- <div class="col d-flex flex-column">
                                    <input class="btn btn-light border rounded" value="Tháng hiện tại" readonly onclick="statistical('{{ $carbon->parse('first day of this month')->toDateString() }}', '{{ $carbon->now('Asia/Ho_Chi_Minh')->toDateString() }}')">

                                    <input class="btn btn-light border rounded" value="Tháng vừa qua" readonly onclick="statistical('{{ $carbon->parse('first day of last month')->toDateString() }}', '{{ $carbon->parse('last day of last month')->toDateString() }}')">

                                    <input class="btn btn-light border rounded" value="3 tháng vừa qua" readonly onclick="statistical('{{ $carbon->parse('first day of 2 months ago')->toDateString() }}', '{{ $carbon->now('Asia/Ho_Chi_Minh')->toDateString() }}')">

                                    <input class="btn btn-light border rounded" value="6 tháng vừa qua" readonly onclick="statistical('{{ $carbon->parse('first day of 5 months ago')->toDateString() }}', '{{ $carbon->now('Asia/Ho_Chi_Minh')->toDateString() }}')">

                                    <input class="btn btn-light border rounded" value="Năm hiện tại" readonly onclick="statistical('{{ $carbon->now('Asia/Ho_Chi_Minh')->year }}-01-01', '{{ $carbon->now('Asia/Ho_Chi_Minh')->toDateString() }}')">

                                    <input class="btn btn-light border rounded" value="Năm trước" readonly onclick="statistical('{{ $carbon->now('Asia/Ho_Chi_Minh')->subYear()->year }}-01-01', '{{ $carbon->now('Asia/Ho_Chi_Minh')->subYear()->year }}-12-31')">
                                ...
                                <div class="col">
                                    <form action="/admin/statistical" id="frmSubmitStatistical" method="post">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="datetime-local" class="form-control" name="end_date" id="end_date" required max="{{ date('Y-m-d') }}">
                                        </div>
                                    </form>
                                </div>
                            </div>
                ...
                <div class="row">
                    ...
                    <div class="col ml-3 bg-white">
                        ...
                        <div class="col-md-12 col-sm-12 ">
                            ...
                            <div class="mt-2" onclick="showPD({{ $v->product_id }})" data-bs-toggle="modal" data-bs-target="#viewProductDetail">
                                <p>{{ $product->find($v->product_id)->name }}: {{ $v->quantity }}</p>
                                @php
                                $quantity_sum = $order_detail->sum('quantity');
                                if ($quantity_sum < 1) { $quantity_sum=1; } @endphp <div class="">
                                    <div class="progress progress_sm hover-percent" style="width: 80%;" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ round(($v->quantity / $quantity_sum) * 100, 2) }}%">
                                        <div class="progress-bar bg-green" role="progressbar" data-transitiongoal="{{ ($v->quantity / $quantity_sum) * 100 }}">
                                        </div>
                                    </div>
                            </div>
                        </div>
            ...
            <table id="datatable" class="table table-striped jambo_table w-100">
                ...
                <tbody>
                    ...
                    <tr class="even pointer text-center">
                        ...
                        <td class=" last">
                            <a href="javascript:;" onclick="showPD({{ $v->product_id }})" data-bs-toggle="modal" data-bs-target="#viewProductDetail">Xem sản phẩm</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
